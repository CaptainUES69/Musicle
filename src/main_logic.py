...
class MainLogic:
    ...
    async def metadata(self, artist_name: str, rounds: int, length_ms: int) -> List[Dict]:
        artist = self.audio.search_artist(artist_name)
        tracks = self.audio.get_tracks_from_artist(artist[0])
        track_list = self.audio.choose_tracks(tracks, rounds)
    ...
    async def metadata_async(self, artist_name: str, rounds: int, length_ms: int) -> List[Dict]:
    ...
    async def trim_track(self, track_id: str | int, length_ms: int) -> BytesIO:
